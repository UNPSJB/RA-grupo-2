        ...
        const cargarDatosBase = async () => {
            ...
            try {
                const response = await api.get(
                    `/docentes/${DOCENTE_ID}/dashboard-estadistico`,
                    {
                        params: {
                            anio: String(anio),
                            periodo: periodo,
                ...
                const basicoExiste =
                    data.estadisticas_basico &&
                    data.estadisticas_basico.promedio_general.length > 0;

                const superiorExiste =
                    data.estadisticas_superior &&
                    data.estadisticas_superior.promedio_general.length > 0;
        ...
        const enriquecerMaterias = async () => {
            ...
            try {
                const promesas = dashboardData.materias_del_ciclo.map(async (materia) => {
                    try {
                        if (
                            materia.cantidad_encuestas !== undefined &&
                            materia.cantidad_encuestas !== 0
                        ) {
                            return materia;
                        }
                        ...
                        const inscriptosReal = resInsc?.data ?? 0;

                        const resEnc = await api.get(`/datos_estadisticos/cantidad_encuestas_completadas`, {
                            params: {
                                id_materia: materia.id,
                                anio,
                                periodo,
                        ...
                        const encuestasReal = resEnc?.data ?? 0;

                        return {
                            ...materia,
                            cantidad_inscriptos: inscriptosReal,
                            cantidad_encuestas: encuestasReal,
                        };
                    } catch {
                        return materia;
                    }
