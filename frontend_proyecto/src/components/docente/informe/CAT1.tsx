...
export default function CategoriaEquipamiento({
  categoria,
  manejarCambio,
  isReadOnly = false,
  respuestas,
  ...
  const pEquipo = useMemo(
    () =>
      categoria.preguntas.find((p) =>
        p.enunciado.toLowerCase().includes("equipamiento")
      ),
    [categoria.preguntas]
  );
  ...
  const pBibliografia = useMemo(
    () =>
      categoria.preguntas.find((p) =>
        p.enunciado.toLowerCase().includes("bibliografia")
      ),
    [categoria.preguntas]
  );

  const respuestaEquipo = pEquipo
    ? respuestas[pEquipo.id]?.texto_respuesta || ""
    : "";
  const respuestaBiblio = pBibliografia
    ? respuestas[pBibliografia.id]?.texto_respuesta || ""
    : "";
  ...
  const enviarRespuestasAlPadre = (
    equipoItems: string[],
    biblioItems: string[]
  ) => {
    if (pEquipo) {
      const equipoConsolidado = equipoItems
    ...
    if (pBibliografia) {
      const biblioConsolidada = biblioItems
        .filter((item) => item.trim() !== "")
        .join("\n");
  ...
  if (!pEquipo || !pBibliografia) {
    return (
      <div className="alert alert-warning">
        Error: Preguntas no asociadas correctamente.
      </div>
    );
  }
    ...
    <Fragment>
      <div className="mb-4">
        <label className="form-label mb-3 fw-bold">
          ...
          {pEquipo.obligatoria && <span className="text-danger ms-1">*</span>}
        </label>
        ...
        <div>
          {itemsEquipo.length > 0 ? (
            ...
            <p className="text-muted fst-italic">No se reportó equipamiento.</p>
          )}
        </div>
      </div>
