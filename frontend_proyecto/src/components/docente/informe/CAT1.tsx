...
interface Props { categoria: CategoriaConPreguntas; manejarCambio: (preguntaId: number, texto: string) => void; }
...
export default function CategoriaEquipamiento({ categoria, manejarCambio }: Props) {
    ...
    const enviarRespuestasAlPadre = (currentFilas: FilaPedido[]) => {
        if (pEquipo) {
            ...
            manejarCambio(pEquipo.id, equipoConsolidado);
        }
        if (pBibliografia) {
            const biblioConsolidada = consolidarRespuestas(currentFilas, 'bibliografia');
            manejarCambio(pBibliografia.id, biblioConsolidada);
        }
    };
