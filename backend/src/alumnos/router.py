...
from src.auth.dependencies import tiene_rol_alumno, tiene_rol_docente
...
def read_encuestas(alumno_id: int, db: Session = Depends(get_db),user: user_schemas.User = Depends(tiene_rol_alumno)):
...
def read_alumno(alumno_id: int, db: Session = Depends(get_db), user: user_schemas.User = Depends(tiene_rol_alumno)):
